...
function buildStructuredSummary(input) {
  ...
  const deadlines = [
    ...input.importantDates.filter(
      (d) => /deadline|dueDate|paymentDate|paymentDeadline|actionDeadline/i.test(d.kind)
    ),
  ...
  const deadlines = dedupeFacts(
    [
      ...u.importantDates.filter(
        (d) => /deadline|dueDate|paymentDate|paymentDeadline|actionDeadline|effectiveDate|endDate/i.test(
          d.kind
        )
...
function buildEvidencePassages(explanation, presentationItems) {
  ...
  const absorb = (facts, evidence) => {
    ...
    for (const e of evidence) {
      ...
      const baseScore = Math.max(...relevant.map(evidenceFactPriority)) + (NOISE_EVIDENCE_RE.test(e.text) ? -80 : 0) + (/total\s+ttc|montant|facture|pr[eé]l[eè]vement|avant\s+le|r[eé]glez|retournez/i.test(
        e.text
      ) ? 15 : 0) - (e.text.length > 160 ? 10 : 0);
