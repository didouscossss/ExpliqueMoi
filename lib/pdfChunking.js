...
export function mergeChunkAnalyses(chunkResults = []) {
  ...
  const primaryAmountValues = usable
  ...
  const uniqueAmounts = [...new Set(primaryAmountValues)];
  ...
  // Une analyse chunkée réussie reste "full" ; pages difficiles = warning, pas faux partial
  const reading_quality =
    usable.length === 0
      ? "partial"
      : usable.some((item) => item.analysis.reading_quality === "failed") &&
          !usable.some((item) => item.analysis.reading_quality === "full")
        ? "partial"
        : "full";
  ...
  const amounts = dedupeByKey(
    usable.flatMap((item) =>
      Array.isArray(item.analysis.amounts) ? item.analysis.amounts : []
    ),
    (item) =>
      `${clean(item?.value)}|${clean(item?.kind)}|${clean(item?.page)}`
  ).slice(0, 24);

  const references = dedupeByKey(
    usable.flatMap((item) =>
      Array.isArray(item.analysis.references) ? item.analysis.references : []
    ),
    (item) => `${clean(item?.value)}|${clean(item?.type)}`
  ).slice(0, 24);

  const persons = dedupeByKey(
    usable.flatMap((item) =>
      Array.isArray(item.analysis.persons) ? item.analysis.persons : []
    ),
    (item) => `${clean(item?.name)}|${clean(item?.role)}`
  ).slice(0, 20);

  const deadlines = dedupeByKey(
    usable.flatMap((item) =>
      Array.isArray(item.analysis.deadlines) ? item.analysis.deadlines : []
    ),
    (item) => `${clean(item?.date)}|${clean(item?.label)}`
  ).slice(0, 12);
  ...
  const risks = dedupeByKey(
    usable.flatMap((item) =>
      Array.isArray(item.analysis.risks) ? item.analysis.risks : []
    ),
    (item) => clean(item?.label || item?.message)
  ).slice(0, 12);

  const contradictions = dedupeByKey(
    usable.flatMap((item) =>
      Array.isArray(item.analysis.contradictions)
        ? item.analysis.contradictions
        : []
    ),
    (item) => clean(item?.message)
  ).slice(0, 8);
