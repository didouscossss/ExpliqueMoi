...
import { mergeDates, extractDatesFromTables } from "./dateNormalization.js";
import {
  determineAnalysisQuality,
  qualityToReadingQuality
} from "./analysisQuality.js";

export const MAX_DOCUMENT_SIZE = 4 * 1024 * 1024;

/** Nombre de pages au-delà duquel on préfère le mode chunked. */
export const DIRECT_PAGE_SOFT_LIMIT = 8;

/** PDF texte léger : on peut envoyer plus de pages en direct (Gemini lit le texte). */
export const DIRECT_TEXT_PAGE_LIMIT = 30;
...
export function planPdfChunks(meta = {}) {
  ...
  // Texte extractible + fichier léger → direct jusqu’à 30 pages (évite N appels Gemini)
  const lightTextPdf =
    !scanned &&
    textLength >= 40 &&
    fileSize <= 800 * 1024 &&
    pageCount <= DIRECT_TEXT_PAGE_LIMIT;

  const canDirect =
    !scanned &&
    fileSize <= DIRECT_SIZE_SOFT_LIMIT &&
    (pageCount <= DIRECT_PAGE_SOFT_LIMIT || lightTextPdf);
  ...
  if (canDirect) {
    return {
      ...
      reason: lightTextPdf ? "light_text_pdf" : "small_text_pdf"
    };
  }
...
export function mergeChunkAnalyses(chunkResults = []) {
  ...
  // Fusion dates : conserve rôles distincts + dates des tableaux
  ...
  const timeline = dedupeByKey(
    ...
    (item) => `${clean(item?.date)}|${clean(item?.label)}|${clean(String(item?.page || ""))}`
  ).slice(0, 24);
  ...
  // Qualité globale : une page secondaire / un chunk « partial » ne dégrade pas tout
  const quality = determineAnalysisQuality(mergedAnalysis, {
    failedPages,
    totalPages: processedPages.length + failedPages.length,
    warnings
  });

  mergedAnalysis.reading_quality = qualityToReadingQuality(quality);
  mergedAnalysis.quality = quality;
  // Ne propager que les warnings importants (pas les notes secondaires)
  mergedAnalysis.warnings =
    quality.status === "warning"
      ? [...new Set([...(quality.reasons || []), ...warnings.filter(isImportantWarning)])]
      : warnings.filter(isImportantWarning);

  return {
    ok: true,
    analysis: mergedAnalysis,
    warnings: mergedAnalysis.warnings,
    failedPages,
    processedPages,
    quality
  };
...
function isImportantWarning(text) {
  const value = clean(text);
  if (!value) return false;

  // Warnings secondaires à ignorer pour le statut global
  if (
    /signature|bas de page|cellule vide|page blanche|note secondaire|détail secondaire|detail secondaire| confiden/i.test(
      value
    )
  ) {
    return false;
  ...
  return /page|illisible|difficile à lire|montant|contradiction|échou|échec|important/i.test(
    value
  );
}
