  ...
  for (const chunk of plan.chunks) {
    ...
    if (remaining < 8000) {
      chunkResults.push({
        ...
        processedPages: [],
        warnings: [
          `Budget temps insuffisant pour les pages ${chunk.startPage} à ${chunk.endPage}.`
        ]
      });
      diagnostics.push({
        step: "chunk_budget_skip",
        label,
    ...
    try {
      ...
      if (chunk.strategy === "page_images" || scanned) {
        ...
        const gemini = await callGeminiForAnalysis(parts, {
          retries: 0,
          maxModels: 2,
          timeoutMs: Math.min(28000, Math.max(8000, deadlineAt - Date.now() - 2000)),
          deadlineAt
        });
...
function parseChunkGemini(gemini, validateResult, hasUsableContent) {
  ...
  try {
    const raw = parseGeminiJson(gemini.rawText);
