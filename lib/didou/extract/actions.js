...
const ACTION_PATTERNS = [
  ...
  {
    /*
     * `\b` juste après un participe accentué ("prié", "demandé")
     * ne matche jamais en JavaScript — voir le commentaire détaillé
     * plus bas sur le motif "OBLIGATION PASSIVE". Remplacé par une
     * négation de lettre explicite.
     */
    re: new RegExp(
      `\\b(?:vous\\s+(?:devez|êtes\\s+prié|etes\\s+prie)|veuillez|prière\\s+de|priere\\s+de|il\\s+vous\\s+est\\s+demandé|il\\s+vous\\s+est\\s+demande)(?![a-zà-ÿ])${TAIL5}`,
      "gi"
    ),
    kind: "request"
  },
   ...
   *
   * `e?s?` optionnel après chaque participe : le sujet réel
   * ("ce document", "cette attestation", "les pièces") impose
   * l'accord du participe passé (transmis / transmise /
   * transmises) — un document au féminin ou au pluriel ne doit
   * pas échapper à la reconnaissance.
   *
   * BUG DE LIMITE DE MOT AVEC ACCENT :
   * `\b` en JavaScript ne considère PAS les lettres accentuées
   * comme des caractères de mot — juste après un participe
   * comme "présenté"/"réglé"/"signé"/"payé"/"adressé" (qui se
   * termine par un "é"), `\b` ne trouve donc jamais de frontière
   * valide et le match échoue silencieusement (l'alternative
   * entière est ignorée). Remplacé par une négation explicite de
   * lettre (accentuée ou non) plutôt que `\b`.
   */

  {
    re: new RegExp(
      `\\b(?:doit|doivent)\\s+(?:etre|être)\\s+(?:transmis|envoye?|envoyé|retourne|retourné|regle|réglé|acquitte|acquitté|complete|complété|rempli|signe|signé|fourni|renvoye|renvoyé|paye|payé|regularise|régularisé|adresse|adressé|restitue|restitué|deposé|depose|presente|présenté)e?s?(?![a-zà-ÿ])${TAIL0_120}`,
      "gi"
    ),
    kind: "request"
  },
   ...
   * =====================================================
   *
   * Phrase quasi systématique des courriers de mise en
   * demeure ("nous vous mettons en demeure de...", "je vous
   * mets en demeure de..."), déjà reconnue par Semantic
   * Relevance (rôle REQUIRED / cible USER) mais jamais
   * capturée ici en amont — elle n'atteignait donc jamais
   * `brain.actions`.
   */

  {
    re: new RegExp(
      `\\b(?:nous\\s+vous\\s+mettons\\s+en\\s+demeure|je\\s+vous\\s+mets\\s+en\\s+demeure)\\b${TAIL0_140}`,
      "gi"
    ),
    kind: "request"
  },
  ...
  {
    re: new RegExp(
      `\\bmerci\\s+de\\s+(?:(?:bien\\s+)?(?:nous\\s+)?(?:transmettre|envoyer|retourner|compléter|completer|signer|fournir|joindre|répondre|repondre|régler|regler|payer|confirmer|contacter|appeler|participer|voter|régulariser|regulariser|rembourser|prendre\\s+rendez[\\s-]?vous|vous\\s+présenter|vous\\s+presenter))\\b${TAIL0_140}`,
      "gi"
    ),
    kind: "request"
  },
  ...
  {
    /*
     * Même bug `\b`/accent que plus haut, mais en tête de motif
     * cette fois : "à" n'est pas un caractère de mot pour `\b`, donc
     * `\b(?:à\s+retourner|...)` ne trouve jamais de frontière valide
     * avant "à retourner"/"à payer" (la forme réellement écrite dans
     * un document français) — seules les variantes sans accent
     * ("a retourner"/"a payer", qui n'apparaissent quasiment jamais
     * dans un vrai texte) pouvaient matcher. Remplacer le `\b` de
     * tête par une négation arrière corrige les deux sens à la fois.
     */
    re: new RegExp(
      `(?<![a-zà-ÿ])(?:à\\s+retourner|a\\s+retourner|à\\s+payer|a\\s+payer|régler|regler|transmettre|envoyer|répondre|repondre|compléter|completer|signer|fournir|joindre|confirmer|contacter|appeler|participer|donner\\s+procuration|voter|régulariser|regulariser|rembourser)\\b${TAIL0_120}`,
      "gi"
    ),
    kind: "action"
  },
...
function containsRealActionVerb(
  ...
  return (
    /\b(?:transmettre|envoyer|retourner|completer|signer|fournir|joindre|repondre|regler|payer|confirmer|contacter|appeler|participer|voter|presenter|prendre\s+rendez[\s-]?vous)\b/.test(
      normalizeText(
        text
      )
    )
...
function calculateConfidence({
  ...
  if (
    /vous\s+devez|veuillez|il\s+vous\s+est\s+demande|vous\s+etes\s+prie|doit\s+(?:etre|être)|doivent\s+(?:etre|être)|mettons\s+en\s+demeure|mets\s+en\s+demeure/.test(
      text
    )
  ) {
    confidence += 15;
