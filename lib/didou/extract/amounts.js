 ...
 * 1 175,00 €
 * 10.640,50 EUR
 * 0,2062 € (prix unitaire, ex. le kWh — toujours à 4 décimales en
 * France, jamais 2)
 *
 * La partie décimale accepte jusqu'à 4 chiffres, pas seulement 2 :
 * un prix unitaire ("0,2062 €") ne correspondait à aucune des deux
 * alternatives (max 2 décimales), donc le moteur retombait sur les
 * 3 DERNIERS chiffres seuls ("062"), lus comme un montant entier —
 * "62,00 €", une valeur totalement fausse et sans rapport avec le
 * vrai prix. Un montant classique à 2 décimales ("14,99 €") reste
 * identique, "1 à 4 chiffres" le couvrant tout autant.
 */
const AMOUNT_RE =
  /(\d{1,3}(?:[ \u00a0.]\d{3})*(?:[.,]\d{1,4})?|\d+[.,]\d{1,4})\s*(?:€|EUR|euros?)(?=\s|$|[.,;:)])/gi;
...
const AMOUNT_EUR_FIRST_RE =
  /(?:€|EUR)\s*(\d{1,3}(?:[ \u00a0.]\d{3})*(?:[.,]\d{1,4})?|\d+[.,]\d{1,4})(?=\s|$|[.,;:)])/gi;
...
function collectAmounts(
  ...
  while ((match = regex.exec(source))) {
    ...
    /*
     * Un prix unitaire à plus de 2 décimales ("0,2062 €", le kWh)
     * ne doit PAS être arrondi à 2 décimales par formatEuro() — ça
     * afficherait "0,21 €", une valeur fausse. On préserve alors la
     * précision d'origine telle qu'écrite dans le document plutôt
     * que de la reformater.
     */
    const decimalDigits =
      (
        String(rawNumber)
