...
export function extractDatesAndPeriods(text) {
  ...
  const pushDate = (
    ...
    dates.push({
       ...
       * detectDateHint) — exposé pour que le rôle final
       * (interpret/roles.js) puisse, lui aussi, distinguer un
       * déclencheur qui décrit CETTE date d'un déclencheur qui,
       * dans la même fenêtre de contexte, décrit en réalité une
       * autre date voisine.
       */
      before:
        snippet.before,
...
function detectDateHint(
   ...
   * Échéance.
   *
   * Ces expressions sont TOUJOURS des préfixes qui introduisent la
   * date qui les suit immédiatement ("avant le X", "jusqu'au X", "à
   * échéance le X") — jamais un mot qui décrit une date déjà passée.
   * On ne les cherche donc que dans le texte qui PRÉCÈDE cette date
   * précise, jamais dans tout le voisinage : sinon, "signé le
   * 01/04/2020, arrivant à échéance le 31/03/2026" faisait aussi
   * passer la date de SIGNATURE (2020, déjà passée) pour une
   * échéance, simplement parce que le mot "échéance" — qui décrit en
   * réalité l'AUTRE date, 2026 — se trouvait dans le même voisinage.
   */
  if (
