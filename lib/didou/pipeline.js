...
export function runDidouPipeline(
  ...
  /*
   * =====================================================
   * PAGE D'ORIGINE DE CHAQUE SIGNAL
   * =====================================================
   *
   * Chaque date / montant / action extrait porte un offset de
   * caractère dans `text` (déjà calculé par les extracteurs pour
   * leurs propres besoins de contexte). On le relie ici au numéro
   * de page physique dont il provient, pour que la structure
   * documentaire (Document Structure Engine — quelle page est une
   * annexe, un document principal, etc.) puisse être utilisée
   * plus tard comme signal de centralité.
   *
   * Purement additif : rien ne consomme encore `item.page` en
   * amont de cette étape, et les items qui n'ont pas d'offset
   * (ou pas de pages, ex. texte collé) restent inchangés.
   */

  attachPageNumbers(
    rawExtraction.dates,
    normalized.pageOffsets
  );

  attachPageNumbers(
    rawExtraction.amounts,
    normalized.pageOffsets
  );

  attachPageNumbers(
    rawExtraction.actionPhrases,
    normalized.pageOffsets
  );
 ...
 brain =
  runBrain({
    ...
    /*
     * Pages structurées conservées jusqu'au Brain.
     *
 ...
 * =====================================================
 *
 * Mutation en place volontaire : les extracteurs (dates.js,
 * amounts.js, actions.js) produisent déjà `index`/`match.index`
 * pour leurs propres besoins de contexte local ; on réutilise cet
 * offset tel quel plutôt que de faire porter cette responsabilité
 * à chaque extracteur individuellement.
 */
function attachPageNumbers(items, pageOffsets) {
  if (!Array.isArray(items) || !Array.isArray(pageOffsets) || !pageOffsets.length) {
    return;
  }
  ...
  for (const item of items) {
    ...
    const page = resolvePageForOffset(pageOffsets, offset);
