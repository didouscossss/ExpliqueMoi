...
const fullContext =
  [
    ...
    .toLowerCase();
  /*
   * "Hausse estimée de X"/"impact estimé de X" est un signal plus
   * précis que le hint générique "invoice_line" (qui se déclenche
   * dès qu'un mot comme "consommation"/"kwh" traîne n'importe où
   * dans le voisinage élargi du montant — vrai ici même pour ce
   * montant-ci, qui n'a pourtant aucun rapport avec une ligne de
   * facture). Vérifié avant les hints pour ne pas être écrasé.
   *
   * Sur `paragraph` (borné au vrai paragraphe), pas `ctx` (fenêtre
   * large) : un document avec plusieurs prix unitaires PUIS, dans
   * un paragraphe séparé, "une hausse estimée de 68,00 €" faisait
   * sinon aussi gagner ce rôle à un prix unitaire voisin sans
   * rapport (le dernier prix du kWh cité juste avant le
   * paragraphe), simplement parce qu'il restait dans le rayon des
   * 120 caractères.
   */
  if (
    /hausse estimée|hausse estimee|impact estimé|impact estime|augmentation estimée|augmentation estimee|économie estimée|economie estimee|baisse estimée|baisse estimee/.test(
      paragraph
    )
  ) {
    return {
      role: "estimatedImpactAmount",
      confidence: 85,
      important: true
    };
  }
...
if (
  ...
  /*
   * =====================================================
   * 6 bis — PRÉLÈVEMENT / PAIEMENT REJETÉ
   * =====================================================
   *
   * "Un prélèvement de 89,90 € ... a fait l'objet d'un rejet" :
   * l'argent n'a PAS été prélevé (contrairement au bloc 6
   * "DÉJÀ PAYÉ/PRÉLEVÉ" juste au-dessus) — la somme reste due,
   * exactement comme un montant impayé classique. Absent
   * jusqu'ici, un avis de rejet de prélèvement (motif bancaire
   * très courant) ne faisait ressortir AUCUN montant principal.
   */
  if (
    /rejet|rejeté|rejete|non honoré|non honore|insuffisance de provision|impayé|impaye|n['’]a pas pu être prélevé|n a pas pu etre preleve|incident de paiement/.test(
      ctx
    ) &&
    /prélèvement|prelevement|virement|paiement|chèque|cheque/.test(
      ctx
    )
  ) {
    return {
  ...
  /*
   * Montant d'une demande de prise en charge / remboursement, y
   * compris quand elle est REJETÉE : "des soins d'orthodontie pour
   * un montant de 1 850,00 €" dans un refus de prise en charge n'est
   * ni un montant dû, ni un remboursement (justement refusé) — mais
   * reste le chiffre le plus important du document pour l'assuré.
   */
  if (
    /demande de prise en charge|demande de remboursement|rejet de (?:votre |la )?demande|refus de (?:votre |la )?demande/.test(
      ctx
    ) &&
    /montant de|pour un montant/.test(
      ctx
    )
  ) {
    return {
...
export function interpretDateRole(
   ...
   */
  if (
    /assemblée|assemblee|convocation|convoqué|convoque|ordre du jour|réunion|reunion|comparaître|comparaitre|rendez-vous|audience|se tiendra|aura lieu/.test(
      ctx
    ) ||
    family === "copropriete"
  ) {
    if (
      /assemblée|assemblee|ag\b|convocation|convoqué|convoque|réunion|reunion|comparaître|comparaitre|rendez-vous|audience|se tiendra|aura lieu/.test(
        ctx
      ) ||
