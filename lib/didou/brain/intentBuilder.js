...
export function buildDocumentIntent({
  ...
  /*
   * Confirmation de résiliation / annulation : le document
   * communique le statut d'une décision prise (résilier,
   * annuler, désactiver), qu'elle vienne de l'utilisateur ou
   * de l'émetteur. Générique à tout type d'abonnement/contrat.
   */
  if (
    /confirmation de resiliation|confirmation d.annulation|votre resiliation|votre contrat (?:est|sera) resilie|votre abonnement (?:est|sera) resilie|votre ligne sera desactivee|demande de resiliation/.test(
      source
    )
  ) {
    decisionScore += 55;
