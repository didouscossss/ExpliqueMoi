...
/* ============================================================
 * 13B. RÔLE STRUCTUREL PAR PAGE
 * ============================================================
 *
 * Document Structure Engine classe chaque page physique (rôle
 * primary/supporting/annex/unknown, sous-type éventuel). Ce
 * n'est PAS une décision : c'est un indice supplémentaire, au
 * même titre que "cette phrase contient le mot annexe" — sauf
 * qu'ici l'indice est structurel (où l'information se trouve
 * dans le document) plutôt que lexical (ce que dit la phrase).
 *
 * Un document sans pages structurées (texte collé, erreur du
 * moteur de structure) donne simplement une Map vide : aucun
 * signal n'est alors appliqué, jamais une pénalité par défaut.
 * ============================================================
 */

function buildPageRoleMap(documentStructure) {
  const pages =
    Array.isArray(documentStructure?.pages)
  ...
  for (const entry of pages) {
    ...
    map.set(pageNumber, {
      ...
      subDocumentType: entry?.subDocumentType || null,
      confidence: safeNumber(entry?.confidence, 0)
    });
  }
...
function inferConditionality(text) {
 ...
 if (
  matchesAny(t, [
    ...
    /*
     * "si" n'est accepté que lorsqu'il introduit
     * réellement une condition exploitable.
     */
    /\bsi vous\b/,
    /\bsi le\b/,
    /\bsi la\b/,
 ...
 * --------------------------------------------------------
 * SIGNAL 3B
 * Action directement liée à la finalité du document.
 *
 * Exemple :
 * Convocation AG
 * → participer à l'Assemblée générale
 *
 * Cette action doit être considérée comme centrale,
...
const documentFamily =
  normalizeText(
    documentContext?.family
  );

const isAssemblyDocument =
  (
    ...
    documentFamily.includes(
      "assemblee"
    )
  );

const isAssemblyParticipationAction =
  matchesAny(
    itemContext,
    [
      /\bparticiper a l[' ]?assemblee\b/,
      /\bparticiper a cette assemblee\b/,
      /\bassister a l[' ]?assemblee\b/,
      /\bnous vous invitons a participer\b/,
      /\bvous etes invite(?:e|s|es)? a participer\b/
    ]
  );

if (
  isAssemblyDocument &&
  isAssemblyParticipationAction
) {
  score += 30;
   ...
   * --------------------------------------------------------
   * SIGNAL 8
   * Rôle structurel de la page d'origine.
   *
   * Signal STRUCTUREL (où l'information se trouve dans le
   * document), complémentaire des signaux lexicaux ci-dessus
   * (ce que dit la phrase). Une date sur une page classée
   * "annexe commerciale" par Document Structure Engine doit
   * peser moins qu'une date identique sur une page du document
   * principal — même si aucun mot-clé "annexe" n'apparaît dans
   * la phrase elle-même.
   *
   * Volontairement prudent :
   * - ignoré si la page n'a pas pu être classée (page inconnue,
   *   pas de structure disponible) ;
   * - ignoré si la confiance de la classification est faible ;
   * - jamais assez fort à lui seul pour faire basculer un score
   *   par ailleurs solide (ampleur comparable aux signaux
   *   lexicaux 4/5/6, pas plus).
   * --------------------------------------------------------
   */

  const pageInfo =
    Number.isFinite(Number(item?.page))
      ? documentContext?.pageRoles?.get(Number(item.page))
      : null;

  if (pageInfo && safeNumber(pageInfo.confidence, 0) >= 70) {
    if (pageInfo.role === "primary") {
      score += 10;
      reasons.push("structural-primary-page");
    } else if (pageInfo.role === "annex") {
      score -= 20;
      reasons.push("structural-annex-page");
    } else if (pageInfo.role === "supporting") {
      score -= 5;
      reasons.push("structural-supporting-page");
    }
  }
...
function inferTarget(text) {
   ...
   */
/*
 * Destinataire explicitement utilisateur.
 *
 * Le simple mot "votre" ne suffit plus :
 * il indique souvent seulement la possession
 * d'un dossier, contrat, logement, compte, etc.
 */
if (
  matchesAny(t, [
    /\bvous devez\b/,
    ...
    /\bvous pouvez\b/,
    /\bvous etes invite(?:e|s|es)?\b/,
    /\bil vous appartient de\b/,
    /\bil vous incombe de\b/,
    /\bvous etes tenu(?:e|s|es)? de\b/,
    /\bvous devez transmettre\b/,
    /\bvous devez fournir\b/,
    /\bvous devez envoyer\b/,
    /\bvous devez retourner\b/,
    /\bvous devez regler\b/,
    /\bvous devez payer\b/,
    /\bvous devez signer\b/,
    /\bvous devez confirmer\b/,
    /\bnous vous mettons en demeure\b/,
    /\bje vous mets en demeure\b/
  ])
) {
  return TARGET.USER;
}
 ...
 * Tiers explicite.
 *
 * On cherche une entité tierce directement reliée
 * à une obligation ou à une action.
 */
if (
  matchesAny(t, [
    /\ble syndic\s+(?:doit|devra|est tenu de|est charge de)\b/,
    /\bl'entreprise\s+(?:doit|devra|est tenue de|est chargee de)\b/,
    /\ble bailleur\s+(?:doit|devra|est tenu de|est charge de)\b/,
    /\ble proprietaire\s+(?:doit|devra|est tenu de|est charge de)\b/,
    /\ble prestataire\s+(?:doit|devra|est tenu de|est charge de)\b/,
    /\bl'assureur\s+(?:doit|devra|est tenu de|est charge de)\b/,
    /\ble locataire\s+(?:doit|devra|est tenu de|est charge de)\b/,
    /\ble vendeur\s+(?:doit|devra|est tenu de|est charge de)\b/,
    /\bl'acheteur\s+(?:doit|devra|est tenu de|est charge de)\b/,
    /\bl'employeur\s+(?:doit|devra|est tenu de|est charge de)\b/,
    /\bla banque\s+(?:doit|devra|est tenue de|est chargee de)\b/,
    /\bl'administration\s+(?:doit|devra|est tenue de|est chargee de)\b/
  ])
) {
...
/*
 * Obligation passive SANS agent nommé.
 *
 * "Ce document doit être transmis à l'administration fiscale
 * avant le..." : la voix passive élide le sujet — mais le
 * document est adressé au destinataire, donc c'est bien LUI
 * qui doit agir, pas l'administration (qui n'est ici que le
 * destinataire de l'envoi, pas l'auteur de l'obligation).
 *
 * On exclut le cas où un agent EST nommé explicitement
 * ("doit être signé PAR le syndic") : là, l'obligation
 * incombe à ce tiers, pas à l'utilisateur.
 */
if (
  matchesAny(t, [
    /\bdoit(?:ent)? etre transmis/,
    /\bdoit(?:ent)? etre envoye/,
    /\bdoit(?:ent)? etre retourne/,
    /\bdoit(?:ent)? etre regle/,
    /\bdoit(?:ent)? etre acquitte/,
    /\bdoit(?:ent)? etre complete/,
    /\bdoit(?:ent)? etre rempli/,
    /\bdoit(?:ent)? etre signe/,
    /\bdoit(?:ent)? etre fourni/,
    /\bdoit(?:ent)? etre paye/,
    /\bdoit(?:ent)? etre regularise/,
    /\bdoit(?:ent)? etre adresse/,
    /\bdoit(?:ent)? etre restitue/,
    /\bdoit(?:ent)? etre depose/,
    /\bdoit(?:ent)? etre presente/
  ]) &&
  !/\bpar (?:le|la|l'|notre|votre) [a-z]/.test(t)
) {
  return TARGET.USER;
}
...
if (
  matchesAny(t, [
    ...
    /*
     * Voix passive — tout aussi fréquente dans les
     * courriers administratifs/fiscaux/juridiques que la
     * voix active ci-dessus ("ce document doit être
     * transmis avant le...", "le formulaire doit être
     * retourné sous 15 jours").
     */
    /\bdoit(?:ent)? etre transmis/,
    /\bdoit(?:ent)? etre envoye/,
    /\bdoit(?:ent)? etre retourne/,
 ...
 if (
  matchesAny(t, [
    ...
    /*
     * Invitations explicites adressées à l'utilisateur.
     *
     * Une invitation à participer n'est pas une
     * obligation, mais reste une action utile.
     */
    /\bnous vous invitons a\b/,
    /\bvous etes invite(?:e|s|es)? a\b/,
    /\bvous etes convie(?:e|s|es)? a\b/
  ])
) {
...
/*
 * La couche Semantic Relevance doit réévaluer
 * elle-même la cible.
 *
 * On ne fait plus confiance automatiquement
 * à item.target provenant d'une couche amont.
 */
const target =
  inferTarget(
    context
...
function inferConsequenceType(text) {
 ...
 if (
  matchesAny(t, [
    ...
    /\bnous vous invitons a participer\b/,
    /\bvous etes invite(?:e|s|es)? a\b/,
    /\bvous devez vous presenter\b/,
    /\bpresence requise\b/,
    /\bpresence obligatoire\b/,
    /\bcomparaitre\b/,
    /\bse rendre a\b/
  ])
) {
  ...
  matchesAny(t, [
    ...
    /\bconfirmer votre\b/,
    /\bconfirmer la\b/,
    /\bretourner votre reponse\b/,
    /\bretourner le formulaire\b/,
    /\bfaire connaitre votre decision\b/,
    /\bindiquer votre choix\b/
  ])
) {
 ...
 if (
  matchesAny(t, [
    ...
    /\bfournir (?:les?|des?) pieces?\b/,
    /\btransmettre (?:les?|des?) pieces?\b/,
    /\benvoyer (?:les?|des?) pieces?\b/,
    /\bjoindre (?:les?|des?) justificatifs?\b/,
    /\bfournir (?:les?|des?) justificatifs?\b/,
    /\btransmettre (?:les?|des?) justificatifs?\b/,
    /\benvoyer (?:les?|des?) justificatifs?\b/
  ])
) {
...
if (
  matchesAny(t, [
    /\bvoter\b/,
    /\bexprimer votre vote\b/,
    /\bexprimer son vote\b/,
    /\bemettre votre vote\b/,
    /\bindiquer votre choix\b/,
    /\bfaire connaitre votre decision\b/,
    /\bse prononcer\b/,
    /\bapprouver ou rejeter\b/,
    /\bvote par correspondance\b/
  ])
) {
...
if (
  matchesAny(t, [
    /\bvous avez droit a\b/,
    /\bvous pouvez demander\b/,
    /\bvous pouvez obtenir\b/,
    /\bvous pouvez beneficier\b/,
    /\bvous beneficiez de\b/,
    /\bdroit a remboursement\b/,
    /\bdroit a indemnisation\b/,
    /\bdroit a recours\b/,
    /\bdroit de contester\b/,
    /\bdroit de demander\b/,
    /\bdroit de consulter\b/,
    /\bdroit de vote\b/
  ])
) {
 ...
 if (
  matchesAny(t, [
    /\bvous perdez le benefice\b/,
    /\bvous risquez de perdre le benefice\b/,
    /\bperte de votre droit\b/,
    /\bperte de vos droits\b/,
    /\bdecheance de votre droit\b/,
    /\bdecheance de vos droits\b/,
    /\bentrainera la perte de\b/,
    /\bentraine la perte de\b/,
    /\bsous peine de perdre\b/
  ])
) {
 ...
 if (
  matchesAny(t, [
    /\bpenalite de retard\b/,
    /\bpenalites de retard\b/,
    /\bmajoration de retard\b/,
    /\bfrais de retard\b/,
    /\bindemnite de retard\b/,
    /\bindemnite forfaitaire\b/,
    /\bsera majore\b/,
    /\bsera majoree\b/,
    /\bentrainera une majoration\b/,
    /\bentraine une majoration\b/,
    /\bsera appliquee une penalite\b/,
    /\bsera applique une penalite\b/,
    /\bsous peine de penalite\b/
  ])
) {
 ...
 if (
  matchesAny(t, [
    /\bsera suspendu\b/,
    /\bsera suspendue\b/,
    /\bpeut etre suspendu\b/,
    /\bpeut etre suspendue\b/,
    /\bentrainera la suspension\b/,
    /\bentraine la suspension\b/,
    /\bsera interrompu\b/,
    /\bsera interrompue\b/,
    /\bentrainera l'interruption\b/,
    /\bentraine l'interruption\b/
  ])
) {
  ...
  matchesAny(t, [
    /\bsera resilie\b/,
    /\bsera resiliee\b/,
    /\bpeut etre resilie\b/,
    /\bpeut etre resiliee\b/,
    /\bentrainera la resiliation\b/,
    /\bentraine la resiliation\b/,
    /\bmettra fin au contrat\b/,
    /\bmet fin au contrat\b/,
    /\ble contrat prendra fin\b/,
    /\ble contrat sera termine\b/
  ])
) {
 ...
 if (
  matchesAny(t, [
    /\bsera renouvele\b/,
    /\bsera renouvelee\b/,
    /\bse renouvelle automatiquement\b/,
    /\bsera automatiquement renouvele\b/,
    /\bsera automatiquement renouvelee\b/,
    /\breconduction automatique\b/,
    /\bsera reconduit\b/,
    /\bsera reconduite\b/,
    /\bse poursuivra par tacite reconduction\b/,
    /\btacite reconduction\b/
  ])
) {
...
function analyzeActionSemantics(
 ...
 /*
 * ========================================================
 * VALIDATION INDEPENDANTE DE LA CIBLE
 * ========================================================
 *
 * Semantic Relevance réanalyse lui-même la cible.
 *
 * Une cible USER fournie par une couche précédente
 * ne doit pas être conservée sans preuve locale.
 */
...
const role =
  inferredRole !== ACTION_ROLE.UNKNOWN
    ? inferredRole
    : ACTION_ROLE.UNKNOWN;
 ...
 * Action obligatoire :
 *
 * Une obligation ne devient une vraie action utilisateur
 * que si Didou a réellement identifié l'utilisateur
 * comme cible.
 *
 * Cela évite qu'une obligation concernant :
 * - une société,
 * - un syndic,
 * - une administration,
 * - un prestataire,
 * soit présentée comme quelque chose que l'utilisateur
 * doit lui-même faire.
 */
if (
  semantics.role ===
...
/*
 * --------------------------------------------------------
 * DATE D'EMISSION — V2.7
 * --------------------------------------------------------
 *
 * Une date devient ISSUE_DATE uniquement si une expression
 * d'émission / création du document est directement reliée
 * à LA date actuellement analysée.
 *
 * Cela évite qu'une autre date située dans le même contexte
 * hérite du rôle ISSUE_DATE.
 */
...
const escapedIssueDate =
  issueDateValue
    ? issueDateValue.replace(
        /[.*+?^${}()|[\]\\]/g,
        "\\$&"
      )
    : "";
...
if (escapedIssueDate) {
  const issueDatePatterns = [
     ...
     * Exemples :
     *
     * Date d'émission : 25/06/2026
     * Date du courrier : 25/06/2026
     * Date de la lettre : 25/06/2026
     */

    new RegExp(
      `\\bdate d[' ]emission\\s*:?\\s*${escapedIssueDate}`,
      "i"
    ),

    new RegExp(
      `\\bdate du courrier\\s*:?\\s*${escapedIssueDate}`,
      "i"
    ),

    new RegExp(
      `\\bdate de la lettre\\s*:?\\s*${escapedIssueDate}`,
      "i"
    ),
     ...
     * Édité le 25/06/2026
     * Édité, le 25/06/2026
     * Établi le 25/06/2026
     * Conçu le 04/06/2024
     */

    new RegExp(
      `\\bemis\\s*,?\\s*le\\s+${escapedIssueDate}`,
      "i"
    ),

    new RegExp(
      `\\bedite\\s*,?\\s*le\\s+${escapedIssueDate}`,
      "i"
    ),

    new RegExp(
      `\\betabli\\s*,?\\s*le\\s+${escapedIssueDate}`,
      "i"
    ),

    new RegExp(
      `\\bconcu\\s*,?\\s*le\\s*:?\\s*${escapedIssueDate}`,
      "i"
    )
  ];
...
if (escapedAnalyzedDate) {
  const endDatePatterns = [
    ...
    new RegExp(
      `\\bpour\\s+se\\s+terminer\\s+le\\s+${escapedAnalyzedDate}`,
      "i"
    ),
...
/*
 * --------------------------------------------------------
 * MISE A DISPOSITION / CONSULTATION — V2.8
 * --------------------------------------------------------
 *
 * Exemple :
 *
 * "les pièces justificatives sont tenues à votre
 * disposition le 20/07/2026"
 *
 * Ce n'est ni :
 * - une réunion,
 * - une échéance,
 * - une date juridique.
 *
 * C'est un événement documentaire secondaire.
 */

const availabilityDateValue =
  normalizeText(
    cleanText(
      item?.date ||
      item?.value ||
...
const escapedAvailabilityDate =
  availabilityDateValue
    ? availabilityDateValue.replace(
        /[.*+?^${}()|[\]\\]/g,
        "\\$&"
      )
    : "";

if (escapedAvailabilityDate) {
  const availabilityDatePatterns = [

    /*
     * à votre disposition le 20/07/2026
     * à votre disposition, le 20/07/2026
     */

    new RegExp(
      `\\ba votre disposition\\b.{0,50}\\b(?:le\\s+)?${escapedAvailabilityDate}`,
      "i"
    ),

    /*
     * mis / mise à disposition le...
     */

    new RegExp(
      `\\bmis(?:e|es|s)? a disposition\\b.{0,40}\\b(?:le\\s+)?${escapedAvailabilityDate}`,
      "i"
    ),
    ...
    new RegExp(
      `\\bdisponible(?:s)?\\b.{0,30}\\b(?:le\\s+)?${escapedAvailabilityDate}`,
      "i"
    ),
    ...
    new RegExp(
      `\\bconsultable(?:s)?\\b.{0,30}\\b(?:le\\s+)?${escapedAvailabilityDate}`,
      "i"
    ),
    ...
    new RegExp(
      `\\bconsultation\\b.{0,30}\\b(?:le\\s+)?${escapedAvailabilityDate}`,
      "i"
    )
  ];
...
if (
  ...
  /*
   * Date directement liée à la réunion :
   * très fortement alignée avec l'intention du document.
   */
  if (
   ...
   * pertinent dans un document de réunion,
   * mais il ne doit pas concurrencer la vraie date
   * de l'assemblée.
   *
   * Exemple :
   * mise à disposition de pièces justificatives.
   */
  if (
  ...
  /*
   * Une deadline peut être utile dans une convocation,
   * sans être la date centrale de la réunion.
   */
  if (
  ...
  const candidates =
    sortBySemanticImportance(
      asArray(actions)
        ...
        /*
         * L'action doit d'abord avoir passé
         * la validation "vraie action utilisateur".
         */
        .filter(
          (item) =>
         ...
         * on exige une cible utilisateur explicite.
         */
        .filter(
          (item) =>
            item?.target ===
        ...
        /*
         * Une action affichée dans le résumé utilisateur
         * doit avoir une pertinence suffisamment forte.
         */
        .filter(
          (item) =>
            safeNumber(
        ...
        /*
         * Une information pure ne doit jamais
         * apparaître dans les actions principales.
         */
        .filter(
          (item) =>
            item?.role !==
 ...
 * ============================================================
 */
function extractExplicitUserActionsFromText(
  text
) {
  const source =
    cleanText(
  ...
  const patterns = [
     ...
     */
    /\bnous vous invitons à participer[^.!?]{0,180}/gi,
    /\bnous vous invitons a participer[^.!?]{0,180}/gi,

    /*
     * Demandes / obligations explicites.
     */
    /\bnous vous demandons de[^.!?]{0,180}/gi,
    /\bvous devez[^.!?]{0,180}/gi,
    /\bveuillez[^.!?]{0,180}/gi,

    /*
     * Possibilités utiles.
     */
    /\bvous pouvez[^.!?]{0,180}/gi
  ];
...
const explicitTextActions =
  extractExplicitUserActionsFromText(
    documentContext?.combinedText
  );
...
debugLog(
  "[SEMANTIC DEBUG ANALYZED ACTIONS]",
  analyzedActions.map(
    (action) => ({
      action:
        action?.action,
      ...
      target:
        action?.target,
      upstreamTarget:
        action?.upstreamTarget,
      inferredTarget:
        action?.inferredTarget,
...
/*
 * ========================================================
 * DEBUG — CLASSEMENT FINAL DES DATES
 * ========================================================
 */

debugLog(
  "[DATE RANKING]",
  asArray(analyzedDates)
    .filter(canBePrimaryDate)
    .map((item) => ({
      date:
        item?.date ||
        item?.value ||
...
debugLog(
  "[PRIMARY DATE SELECTED]",
  {
    ...
    relevance:
      primaryDate?.relevance ||
      primaryDate?.semanticRelevance ||
      null
  }
);
  const primaryAmount =
...
debugLog(
  "[SEMANTIC DEBUG SELECTED ACTIONS]",
  userActions
);
