 ...
 * --------------------------------------------------------
 *
 * Une date ne devient MEETING_DATE que si la date est
 * directement reliée à l'événement.
 *
 * La simple présence de mots comme "assemblée",
 * "convocation" ou "réunion" quelque part dans le contexte
 * ne suffit plus.
 */
...
const meetingNearDatePatterns =
  escapedDate
    ? [
        new RegExp(
          `\\b(?:assemblee|reunion|rendez-vous|rendez vous|audience|entretien)\\b.{0,80}${escapedDate}`,
          "i"
        ),

        new RegExp(
          `\\b(?:convoquee?|convocation)\\b.{0,80}${escapedDate}`,
          "i"
        ),

        new RegExp(
          `${escapedDate}.{0,80}\\b(?:assemblee|reunion|rendez-vous|rendez vous|audience|entretien)\\b`,
          "i"
        ),

        new RegExp(
          `\\b(?:se tiendra|aura lieu|est prevu)\\b.{0,80}${escapedDate}`,
          "i"
        )
      ]
    : [];
...
if (
  meetingNearDatePatterns.length > 0 &&
  matchesAny(
    localContext,
    meetingNearDatePatterns
  )
) {
