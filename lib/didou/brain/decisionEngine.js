 ...
 * =====================================================
 *
 * Architecture compatible avec Brain Index V7.
 *
 * Le moteur peut fonctionner en deux modes :
 *
 * PASS 1
 * ------
 * Brain sans consensus.
 * Comportement proche du Decision Engine V1.
 *
 * PASS 2
 * ------
 * Brain avec consensus.
 * Le type documentaire final devient prioritaire
 * pour sélectionner :
 *
 * - intention
 * - situation
...
export function runDecisionEngine(
  ...
  /*
   * =====================================================
   * 2 — EVENEMENT / SITUATION PRINCIPALE
   * =====================================================
   */
  ...
  if (
     ...
     * PASS 2 :
     *
     * le Consensus a arbitré la situation.
     * Il devient donc prioritaire.
     */
...
function pickPrimaryAmount({
  ...
  if (
    ...
    /*
     * En PASS 1 on conserve le comportement V1.
     */
  ...
  /*
   * En PASS 2 on refuse un montant qui n'a plus
   * suffisamment de cohérence avec le document final.
   */
...
function shouldSuppressPrimaryAmount(
  ...
  /*
   * Une convocation AG contient énormément de sommes :
   *
   * budgets
   * travaux
   * devis
   ...
   * honoraires
   *
   * mais aucune de ces sommes n'est forcément
...
function pickPrimaryDate({
   ...
   * ne doit pas gagner simplement grâce à confidence=100.
   */
...
function scoreDateForContext({
  ...
  if (
     ...
     * mais elle n'est pas la date principale d'une AG.
     */
...
function determineActionRequired({
   ...
   * =====================================================
   *
   * Une convocation implique généralement une
   * attention ou participation possible.
   *
   * Si le Consensus n'a pas explicitement dit false,
   * on considère l'action comme requise.
   */
...
/**
 * =====================================================
 * ACTION COMPATIBLE AVEC CONTEXTE ?
 * =====================================================
 */

function isActionCompatibleWithContext({
  action,
  context
}) {
  return Boolean(
...
function normalizeActionForContext({
  ...
  if (
     ...
     * Exemple réel :
     *
     * "vous devez vous adresser à la société dont
     * la marque et les coordonnées se trouvent..."
     *
     * Cette phrase vient d'une annexe / devis.
     */
    ...
    if (
      ...
      /societe|marque|coordonnees/.test(
        text
      )
    ) {
      return null;
     ...
     *
     * Cela concerne seulement le vote par
     * correspondance.
     */
    ...
    if (
      ...
      (
        ...
        /intention de vote|resolution/.test(
          text
        )
      )
    ) {
    ...
    if (
      ...
      return {
        ...
        action:
          "Participer à l’assemblée générale ou prendre connaissance des modalités de participation.",
    ...
    if (
      ...
      return {
        ...
        action:
          "Consulter les modalités de pouvoir ou de procuration si vous ne pouvez pas participer.",
    ...
    /*
     * Une phrase inconnue n'est pas transformée
     * en obligation utilisateur.
     */
  ...
  if (
    ...
    /*
     * Actions financières historiques parasites.
     */

    if (
      /payer|regler|versement|prelevement|remboursement|montant du/.test(
        text
      )
    ) {
      return false;
    ...
    /*
     * Actions liées à la réunion.
     */

    if (
      /participer|assister|voter|vote|procuration|pouvoir|correspondance|assemblee|reunion|syndic/.test(
        text
      )
    ) {
      return true;
    ...
    /*
     * Une phrase très générique n'est pas retenue
     * automatiquement sur une AG.
     */
