...
function determineActionRequired({
  ...
  /*
   * =====================================================
   * 1 — SEMANTIC RELEVANCE — AUTORITE EN PASS 2
   * =====================================================
   *
   * En PASS 2, Semantic Relevance a déjà analysé les
   * actions et éliminé les actions parasites.
   *
   * Une action n'est "requise" que si elle est :
   *
   * - destinée à l'utilisateur ;
   * - explicitement REQUIRED ;
   * - suffisamment forte sémantiquement.
   *
   * Une action OPTIONAL ou RECOMMENDED ne signifie
   * donc pas actionRequired = true.
   */
  ...
  if (
    ...
    /*
     * Semantic Relevance est disponible et aucune
     * vraie action obligatoire utilisateur n'a survécu.
     *
     * Le Consensus, l'intention ou l'événement historique
     * ne doivent pas pouvoir recréer artificiellement
     * une obligation.
     */
  ...
  /*
   * =====================================================
   * 2 — CONSENSUS — FALLBACK
   * =====================================================
   *
   * Utilisé seulement lorsque Semantic Relevance
   * n'est pas disponible.
   */

  if (
    context.final &&
    typeof context?.actionRequired ===
      "boolean"
  ) {
    return context.actionRequired;
  }
  ...
  if (
    primaryEvent?.actionRequired === true
  ) {
    return true;
  }

  if (
    primaryEvent?.actionRequired === false
  ) {
    return false;
  }
   ...
   * =====================================================
   *
   * Fallback utilisé principalement en PASS 1.
   */
  ...
  const hasRequiredAction =
    actions.some(
      (action) => {
        ...
        /*
         * Une recommandation ou une option
...
/*
 * En PASS 2, une action Brain doit être confirmée
 * par Semantic Relevance.
 *
 * Une action historique non retrouvée sémantiquement
 * ne doit plus remonter simplement grâce à sa confiance.
 */
if (
  !semanticInfo
) {
...
function shouldRejectActionBySemanticRelevance(
   ...
   * =====================================================
   *
   * En PASS 2, une action affichée à l'utilisateur
   * doit être explicitement destinée à l'utilisateur.
   */

/*
 * Un tiers explicite est rejeté.
 *
 * En revanche, UNKNOWN n'est pas rejeté ici :
 * Semantic Relevance peut avoir une bonne action
 * sans avoir réussi à déterminer parfaitement la cible.
 *
 * Les contrôles de rôle et de score restent actifs.
 */
if (
  target === "thirdparty" ||
  target === "organization"
