...
/*
 * Deuxième passe :
 * résoudre certains UNKNOWN grâce
 * à la continuité documentaire.
 *
 * Exemple :
 *
 * resolutions
...
/**
 * =====================================================
 * CONTINUITE ENTRE PAGES
 * =====================================================
 *
 * Cette passe ne modifie QUE les pages UNKNOWN.
 *
 * Une page inconnue peut hériter de la classification
 * de ses deux voisines uniquement si celles-ci sont
 * parfaitement cohérentes entre elles.
 *
 * On préfère laisser UNKNOWN plutôt que deviner.
 */

function resolvePageContinuity(
  pages
) {
  if (
    !Array.isArray(pages) ||
  ...
  return pages.map(
    (page, index) => {
      /*
       * Une page déjà classée reste intacte.
       */
      ...
      /*
       * Les deux voisins doivent eux-mêmes
       * avoir une classification réelle.
       */
      ...
      /*
       * Le sous-document doit également être
       * exactement le même.
       */

      if (
        !previous.subDocumentType ||
        previous.subDocumentType !==
          next.subDocumentType
      ) {
        return page;
      }

      /*
       * On exige une confiance minimale
       * des deux côtés.
       */
       ...
       *
       * La confiance reste inférieure à celle
       * d'une classification directe.
       */
      ...
      return {
        ...
        reasons: [
          ...
          "inherited-from-page-continuity"
        ],
