...
debugLog(
  "[DIDOU STRUCTURE TEST]",
  {
    pageCount:
      verifiedBrain.documentStructure?.pageCount,

    diagnostics:
      verifiedBrain.documentStructure?.diagnostics,

    segments:
      verifiedBrain.documentStructure?.segments,

    inferredPages:
      verifiedBrain.documentStructure?.pages
        ?.filter(
          (page) =>
            page.inferred === true
        )
...
try {
  verifiedBrain.semanticRelevance =
    ...
    text,
      ...
      /*
       * -----------------------------------------------
       * STRUCTURE DOCUMENTAIRE (par page)
       * -----------------------------------------------
       *
       * Diagnostic uniquement à l'étape 2.5, mais déjà
       * disponible ici : sert de signal de centralité
       * supplémentaire (une page classée "annexe" ne doit
       * pas peser autant qu'une page du document principal),
       * jamais de décision seule.
       */
