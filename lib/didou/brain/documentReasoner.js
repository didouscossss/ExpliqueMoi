...
function buildBrainActions(
  ...
  for (
    ...
    /*
     * Phrase englobante (sujet + modal + verbe).
     *
     * `action.action` reste la phrase courte utilisée pour
     * l'affichage ; `context` porte l'information dont les
     * couches en aval (Semantic Relevance) ont besoin pour
     * déterminer la cible et le rôle réels de l'action —
     * sans quoi "participer..." perd le "Vous pouvez" ou
     * "Vous devez" qui le précédait dans le texte source.
     */

    action.context =
      item?.sentence &&
      item.sentence !== phrase
        ? truncate(
            item.sentence,
            240
          )
