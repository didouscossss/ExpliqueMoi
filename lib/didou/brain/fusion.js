...
export function fuseBrainAndAdapted({
  ...
  result.brainFusion = {
    ...
    version:
      "4.3-contextual-actions-warnings",
...
function mapDecisionDate(
  ...
  if (isGeneralMeeting) {
    return fuseGeneralMeetingActions(
      rawActions
    );
  }
...
function fuseGeneralMeetingActions(
  actions
) {
  const result = [];
  const seen = new Set();
  ...
  for (
    ...
    if (
      /vous devez vous adresser/.test(text) ||
      /vous adresser a la societe/.test(text) ||
      /s'adresser a la societe/.test(text) ||
      /societe dont la marque/.test(text) ||
      /coordonnees.*recto/.test(text) ||
      /sur le devis/.test(text) ||
      /payer|regler|prelevement|remboursement|versement/.test(text) ||
      /signer toutes les pages/.test(text)
    ) {
      continue;
    }
    ...
    if (
      /vote par correspondance/.test(text) ||
      /bulletin de vote/.test(text) ||
      /voter par correspondance/.test(text) ||
      /cocher.*resolution/.test(text)
    ) {
      canonical =
        "Consulter les modalités de vote par correspondance si vous souhaitez utiliser cette possibilité.";
    } else if (
      /procuration/.test(text) ||
      /\bpouvoir\b/.test(text) ||
      /mandat/.test(text)
    ) {
      canonical =
        "Consulter les possibilités de pouvoir ou de procuration prévues dans la convocation si vous ne pouvez pas participer.";
    } else if (
      /participer/.test(text) ||
      /assister/.test(text) ||
      /presence/.test(text)
    ) {
      canonical =
...
function isLegacyGeneralMeetingWarning(
  ...
  return (
    /plusieurs dates limites differentes/.test(text) ||
    /plusieurs dates differentes apparaissent/.test(text) ||
    /verifiez laquelle s'applique/.test(text) ||
    /verifiez laquelle s applique/.test(text) ||
    /date.*2019/.test(text) ||
    /date.*arrete/.test(text) ||
    /date.*decret/.test(text) ||
    /date.*annexe/.test(text)
  );
}
