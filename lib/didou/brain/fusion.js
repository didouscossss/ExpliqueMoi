...
function pickGeneralMeetingDate({
  ...
  const candidates = [];
console.log(
  "🧩 [FUSION AG] ADAPTED MAIN DATE =",
  adapted?.mainDate || null
);

console.log(
  "🧩 [FUSION AG] ADAPTED FACTS =",
  adapted?.importantFacts || []
);
  console.log(
  "🕐 [FUSION AG] BRAIN DATES =",
  brain?.dates || null
);

console.log(
  "🕐 [FUSION AG] DECISION PRIMARY DATE =",
  decision?.primaryDate || null
);

console.log(
  "🕐 [FUSION AG] BRAIN EVENTS =",
  brain?.events ||
  brain?.event ||
  null
);
   ...
   * - le meaning enrichi
   *
   * afin de ne plus perdre l'heure pendant la fusion.
   */
