...
const DATE_MEANING_BY_ROLE = {
  ...
  startDate: "Date de début ou d’entrée en vigueur"
};
...
const AMOUNT_MEANING_BY_ROLE = {
  ...
  financialResult: "Résultat financier de l’exercice",
  penalty: "Montant d’une amende ou d’une pénalité",
  claimAmount: "Montant concerné par la demande",
  estimatedImpactAmount: "Impact estimé sur votre facture"
};

/*
 * `label` est lui aussi un texte destiné à l'utilisateur — le
 * nom technique du rôle interne (ex. "debitDate", "refundAmount")
 * ne doit jamais s'afficher tel quel.
 */

const DATE_LABEL_BY_ROLE = {
  issueDate: "Date d’émission",
  deadline: "Date limite",
  meetingDate: "Date du rendez-vous",
  paymentDate: "Date du paiement",
  debitDate: "Date du prélèvement",
  refundDate: "Date du remboursement",
  coveredPeriod: "Période",
  startDate: "Date de début"
};

const AMOUNT_LABEL_BY_ROLE = {
  amountDue: "Montant à payer",
  paymentAmount: "Montant payé",
  paidAmount: "Montant payé",
  refundAmount: "Montant remboursé",
  refundedAmount: "Montant remboursé",
  automaticDebitAmount: "Montant prélevé",
  installmentAmount: "Échéance",
  salary: "Rémunération",
  deposit: "Dépôt de garantie",
  financialResult: "Résultat de l’exercice",
  penalty: "Amende",
  claimAmount: "Montant demandé",
  estimatedImpactAmount: "Impact estimé"
};
...
function describeDateLabel(date) {
  return DATE_LABEL_BY_ROLE[date?.role] || "Date";
}

function describeAmountLabel(amount) {
  return AMOUNT_LABEL_BY_ROLE[amount?.role] || "Montant";
}
...
export function adaptGeneric(ctx) {
  ...
  if (mainDate) {
    importantFacts.push({
      kind: "date",
      label: describeDateLabel(mainDate),
      value: mainDate.raw,
  ...
  if (mainAmount) {
    importantFacts.push({
      kind: "amount",
      label: describeAmountLabel(mainAmount),
  ...
  return {
    ...
    mainDate: mainDate
      ? {
          ...
          confidence: mainDate.confidence || 0,
          /*
           * Heure/lieu d'un rendez-vous (audience, rendez-vous
           * médical, entretien...) hors assemblée générale de
           * copropriété (seul cas déjà couvert par
           * adapters/condoMeeting.js) : sans ça, un rendez-vous "hors
           * copropriété" ressortait toujours sans heure ni lieu,
           * même quand l'un ou l'autre était écrit juste à côté de
           * la date dans le document.
           */
          time:
            mainDate.role === "meetingDate"
              ? extractMeetingTimeFromContext(mainDate.context)
              : null,
          place:
            mainDate.role === "meetingDate"
              ? extractMeetingPlaceFromContext(mainDate.context)
              : null
        }
      : null,
