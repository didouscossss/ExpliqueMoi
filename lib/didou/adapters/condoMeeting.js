...
import { toDigitDate } from "../normalize/text.js";
...
debugLog(
  "🕒 [CONDO MEETING] TEXTE AUTOUR DES HEURES =",
  String(text || "")
    .match(/.{0,100}\b\d{1,2}\s*(?:h|:)\s*\d{0,2}.{0,100}/gi)
);
  /*
   * =====================================================
   * LIEU
 ...
 mainDate: meetingDate
  ? {
      ...
      confidence:
        Number(
          meetingDate?.confidence || 95
        ),

      source:
        "condo-meeting-adapter"
    }
  : null,
  ...
  const scored =
    list
      .map((date) => {
        ...
        /*
         * "Assemblée Générale ORDINAIRE du..."/"...EXTRAORDINAIRE
         * du..." : le qualificatif (quasi systématique dans un
         * vrai titre de PV/convocation) s'intercale entre
         * "générale" et "du" — le motif doit le tolérer, sinon il
         * ne matche jamais un vrai document.
         */
        if (
          /assemblee generale(?:\s+(?:ordinaire|extraordinaire|mixte))?\s+du\b/.test(
            context
          )
        ) {
          score += 220;
...
function scoreMeetingDateInSource(
  ...
  while (
    ...
    if (
      ...
      !/se tiendra|date de l assemblee|assemblee generale(?:\s+(?:ordinaire|extraordinaire|mixte))?\s+du\b/.test(
        local
      )
    ) {
      score -= 240;
...
function extractMeetingTime(
  ...
  /*
   * `\b` juste avant "à" échoue toujours : "à" n'est pas un
   * caractère de mot pour `\b` en JavaScript, donc `\b(?:à|a)` ne
   * trouve jamais de frontière valide quand "à" (la forme
   * réellement écrite) est précédé d'un espace — seule la variante
   * "a" sans accent (quasi absente d'un vrai document) passait.
   * `(?<![a-zà-ÿ])` remplace `\b` en tête pour corriger les deux cas.
   */
  const patterns = [
    /(?:assemblée générale|assemblee generale|assemblée|assemblee|AG|réunion|reunion)[\s\S]{0,180}?(?<![a-zà-ÿ])(?:à|a)\s*(\d{1,2})\s*h\s*(\d{2})?\b/i,
    ...
    /(?<![a-zà-ÿ])(?:à|a)\s+(\d{1,2})\s*h\s*(\d{2})?\b/i,

    /(?<![a-zà-ÿ])(?:à|a)\s+(\d{1,2}):(\d{2})\b/i
  ];
...
function extractMeetingPlace(
   ...
   * se tiendra à ...
   *
   * `\b` juste avant "à" ne trouve jamais de frontière valide (voir
   * extractMeetingTime plus haut) : `(?<![a-zà-ÿ])` le remplace ici.
   * Négation `(?!\d{1,2}\s*(?:h...|:...))` : sans elle, "se tiendra
   * le 14/10/2026 à 18h30" (heure, pas lieu) sans "Lieu :" ni second
   * "à" ensuite se faisait passer pour le début du lieu ("18h30,
   * Salle des fêtes..." au lieu de juste "Salle des fêtes...").
   */
  collectPlaceCandidates(
    source,
    /\b(?:se tiendra|aura lieu)[\s\S]{0,120}?(?<![a-zà-ÿ])(?:à|a|au|aux)\s+(?!\d{1,2}\s*(?:h\s*\d{0,2}\b|:\s*\d{2}\b))/gi,
    150,
    100,
    candidates
  );
  ...
  /*
   * (se sont) réunis (en assemblée générale) à ...
   *
   * Formulation typique d'un PROCÈS-VERBAL (rédigé après la
   * réunion, contrairement à une convocation) : "les copropriétaires
   * ... se sont réunis en assemblée générale à [lieu]". Absente des
   * deux motifs précédents ("se tiendra"/"aura lieu"/"Lieu :"), donc
   * aucun lieu n'était jamais détecté sur ce type de document. Même
   * négation anti-heure que ci-dessus.
   */
  collectPlaceCandidates(
    source,
    /\br(?:é|e)uni[a-z]*[\s\S]{0,60}?(?<![a-zà-ÿ])(?:à|a|au|aux)\s+(?!\d{1,2}\s*(?:h\s*\d{0,2}\b|:\s*\d{2}\b))/gi,
    150,
    100,
    candidates
  );
  ...
  /*
   * Ligne d'adresse juste après la date/heure, sans préposition
   * introductive ("Le mercredi 14 octobre 2026 à 18h30\nSalle des
   * fêtes, 2 rue de la Mairie, 86000 Poitiers") : très courant dans
   * une convocation qui ne répète pas "Lieu :". On ne déclenche que
   * si la ligne suivante ressemble vraiment à un lieu (mot-clé de
   * bâtiment/salle ou numéro de rue), pour éviter de capturer une
   * phrase quelconque.
   */
  collectPlaceCandidates(
    source,
    /\d{1,2}\s*h\s*\d{0,2}\s*\n\s*(?=\d{1,4}\s+(?:rue|avenue|av|boulevard|bd|route|chemin|impasse|allee|place|quai|cours)\b|(?:salle|mairie|h[oô]tel|centre|residence|résidence|local|agence)\b)/gi,
    150,
    90,
    candidates
  );
...
/*
 * Un PDF extrait "en mise en page" (pdftotext -layout, cas courant
 * des PV/convocations scannés puis convertis) coupe les lignes à la
 * largeur de colonne, au milieu d'un nom de lieu ("...86270 La" /
 * "Roche Posay ( initialement prévue...") plutôt qu'entre deux
 * informations distinctes. Rejoindre ces lignes avec ", " comme les
 * autres produit "La, Roche Posay" — une virgule au milieu d'un seul
 * nom de lieu. Si la ligne précédente se termine par un article/
 * préfixe de nom de lieu français courant (trop court pour être la
 * fin d'une ligne d'adresse à lui seul), c'est presque toujours une
 * coupure de mise en page : on rejoint alors par un simple espace.
 */

const PLACE_NAME_PREFIXES =
  new Set([
    "la",
    "le",
    "les",
...
function cleanPlaceCandidate(
  ...
  /*
   * Une ligne blanche (saut de paragraphe) marque quasi toujours la
   * fin du bloc d'adresse et le début d'un tout autre sujet ("...
   * 86000 Poitiers\n\nL'ordre du jour sera le suivant :"). Sans
   * cette coupure, ce nouveau paragraphe continuait d'être englouti
   * ligne par ligne comme s'il faisait partie du lieu.
   */
  text =
    text.split(
      /\n\s*\n/
  ...
  for (
    ...
    /*
     * Parenthèse ouvrante : le plus souvent une précision annexe
     * ("... 86270 La Roche Posay ( initialement prévue à la
     * ACROPOLYA Salle Cristal ... )" — le lieu initialement prévu,
     * mais pas celui où l'assemblée s'est réellement tenue). On
     * garde ce qui précède la parenthèse et on arrête là.
     */
    const parenIndex =
      line.indexOf(
        "("
