...
function detectGlobalPaymentStatus(text) {
   ...
   */
  if (
    /facture acquittée|facture acquittee|déjà payé|deja paye|déjà réglé|deja regle|paiement reçu|paiement recu|a été prélevé|a ete preleve|déjà prélevé|deja preleve/.test(
      source
    )
  ) {
    return "already_paid";
   ...
   */
  if (
    /sera prélevé|sera preleve|prélèvement automatique|prelevement automatique|prélevé automatiquement|preleve automatiquement|nous prélèverons|nous preleverons|sera débité|sera debite|prélèvement prévu|prelevement prevu/.test(
      source
    )
  ) {
   ...
   */
  if (
    /montant à payer|montant a payer|net à payer|net a payer|reste à payer|reste a payer|montant dû|montant du|total à régler|total a regler|à régler avant|a regler avant|payable sous|payable avant/.test(
      source
    )
  ) {
    return "to_pay";
...
function pickAmountForStatus(
  ...
  if (status === "refund_expected") {
    return (
      ...
      findAmountNearPhrase(
        list,
        fullText,
        /nous vous rembourserons|remboursement|à vous rembourser|a vous rembourser/
      )
    );
  }
  ...
  if (status === "already_paid") {
    return (
      findAmountByContext(
        list,
        /déjà payé|deja paye|payé|paye|réglé|regle|prélevé|preleve|acquitt/
      ) ||
      null
    );
  }
  ...
  if (status === "automatic_debit") {
    return (
      findAmountByContext(
        list,
        /prélèvement|prelevement|sera prélevé|sera preleve|sera débité|sera debite/
      ) ||
      list.find(
        (amount) =>
          amount.role === "amountDue"
  ...
  if (status === "to_pay") {
    return (
      ...
      findAmountByContext(
        list,
        /montant à payer|montant a payer|net à payer|net a payer|reste à payer|reste a payer|total ttc|montant dû|montant du|à régler|a regler/
      ) ||
      null
    );
  }
...
/**
 * Recherche un montant par son contexte local.
 */
