...
export function adaptInvoice(ctx) {
  ...
  for (const a of amounts) {
    debugLog({
      ...
      hints: a.hints,
      line: a.line,
      before: String(a.before || "").slice(-180),
      after: String(a.after || "").slice(0, 180)
    });
  }
  ...
  debugLog("MAIN EVENT", {
    ...
    trigger: mainEvent?.trigger,
    amount: mainEvent?.amount?.value,
    score: mainEvent?.score
  });
  let effectiveStatus =
...
/*
 * =====================================================
 * FALLBACK REMBOURSEMENT
 * =====================================================
 *
 * Si le statut remboursement est certain mais que
 * l'événement n'a pas réussi à rattacher le montant,
 * on utilise la position locale fournie par amounts.js.
 */
...
if (
  ...
  amount =
    pickRefundAmountFromLocalContext(
      amounts
    );
}
...
function pickIssuer(
  ...
  if (!list.length) {
    return pickIssuerFromHeader(source);
  }

  /*
   * Les premières lignes ont beaucoup plus
   * de chances de contenir l'émetteur.
   */
  const header =
    source
      .split(/\n/)
  ...
  const scored =
    list
      .map((organization) => {
        ...
        /*
         * Formes juridiques seules :
         * pas un nom d'émetteur.
         */
        if (
          /^(sa|sas|sarl|eurl|sci|sasu|selarl|sca|snc)$/i.test(
            organization
          )
        ) {
          return null;
        ...
        if (headerIndex >= 0) {
          ...
          /*
           * Plus c'est proche du début,
           * plus c'est probablement l'émetteur.
           */
          if (headerIndex < 80) {
            score += 80;
          } else if (
        ...
        /*
         * =================================================
         * 2 — POSITION DANS LE DOCUMENT COMPLET
         * =================================================
         */
        ...
        /*
         * =================================================
         * 4 — CONTEXTE ENTREPRISE / FACTURE
         * =================================================
         */

        const contexts =
          findOrganizationContexts(
            normalized,
            org,
            120
          );
        ...
        for (
          ...
          if (
            /facture|service client|espace client|contact|fournisseur|societe|entreprise|editeur|emetteur/.test(
              context
            )
          ) {
            score += 30;
          ...
          if (
            /siren|siret|rcs|siege social|capital social/.test(
              context
            )
          ) {
            /*
             * Une mention légale avec le nom
             * complet de l'entreprise est aussi
             * un bon signal d'identité.
             */
            score += 15;
          }
          ...
          if (
            /destinataire|client|adresse de facturation|adresse du client|monsieur|madame|mme|mr|m\./.test(
              context
            )
          ) {
            score -= 45;
          ...
          /*
           * Un nom situé uniquement dans une adresse
           * ne doit pas gagner.
           */
          if (
            /rue|avenue|boulevard|chemin|impasse|allee|allée|route|code postal|cedex/.test(
              context
            )
          ) {
            score -= 30;
...
/**
 * Fallback :
 * cherche un nom d'organisation probable
 * dans le début du document.
 */
function pickIssuerFromHeader(
  text
) {
  const headerLines =
    String(text || "")
  ...
  /*
   * On privilégie une ligne courte ressemblant
   * à un nom de marque / entreprise.
   */
  for (
    const line
    of headerLines
    ...
    if (
      ...
      !/\d{5}/.test(line) &&
      !/@/.test(line) &&
      !/facture|date|numero|n°|client|adresse/i.test(
        line
      )
    ) {
      return line;
...
/**
 * Compte les occurrences d'un texte.
 */
...
/**
 * Récupère plusieurs petits contextes
 * autour du nom de l'organisation.
 */
function findOrganizationContexts(
  source,
  organization,
  radius = 120
) {
...
/**
 * =====================================================
 * MONTANT DE REMBOURSEMENT — CONTEXTE LOCAL
 * =====================================================
 */

function pickRefundAmountFromLocalContext(
  amounts,
  ...
  for (const amount of list) {
    ...
    /*
     * On regarde surtout la FIN de ce qui précède
     * le montant.
     */
    const nearBefore =
      before.slice(-140);
    ...
    if (!alreadyRefunded) {
      ...
      if (
        /remboursement prevu|remboursement a venir/.test(
          local
        )
      ) {
        score += 130;
      ...
      if (
        /avoir en votre faveur|credit en votre faveur|solde crediteur/.test(
          local
        )
      ) {
        score += 120;
    ...
    if (alreadyRefunded) {
      if (
        /vous avez ete rembourse|remboursement effectue|a ete rembourse/.test(
          local
        )
      ) {
        score += 150;
    ...
    /*
     * Le rôle donné par roles.js est un bonus,
    ...
    /*
     * =================================================
     * ÉLIMINATION DES FAUX CANDIDATS
     * =================================================
     */

    if (
      amount?.role === "installmentAmount" ||
    ...
    if (
      amount?.role === "invoiceLineAmount"
    ) {
      score -= 70;
    }

    /*
     * Capital social / montants légaux.
     */
    if (
      /capital social|au capital de|capital de la societe/.test(
        local
      )
    ) {
      score -= 300;
    ...
    /*
     * Si la phrase de remboursement apparaît seulement
     * APRÈS le montant, ce montant est probablement
     * une valeur précédente (ex. 1 175 €).
     */
    const nearAfter =
      after.slice(0, 120);

    if (
      /nous vous rembourserons|vous serez rembourse/.test(
        nearAfter
      ) &&
      !/nous vous rembourserons|vous serez rembourse/.test(
        local
...
function cleanContext(
  ...
  /*
   * Ces fenêtres de contexte concatènent souvent plusieurs
   * phrases avant le fait recherché (montant/date) — afficher
   * les trois phrases précédentes en guise d'"explication"
   * n'aide pas l'utilisateur et peut même l'induire en erreur
   * (une pénalité mentionnée dans une phrase précédente sans
   * rapport avec LE montant qui suit). On ne garde que la
   * dernière phrase : c'est celle qui contient le fait
   * lui-même dans l'immense majorité des cas, ce contexte
   * étant construit comme `avant + fait + après`.
   */
  const sentences =
    full.split(
      /(?<=[.!?])\s+/
