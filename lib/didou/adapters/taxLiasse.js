...
function findFormReference(
  ...
  const patterns = [
    ...
    /*
     * Annexes de liasse fiscale (bilan / compte de résultat).
     * 2032 (détermination du résultat fiscal) et 2033 (liasse
     * simplifiée) sont les plus fréquentes et s'utilisent
     * couramment sans suffixe ("formulaire 2032"), on les
     * accepte donc nues. Le reste de la plage (2034-2039,
     * 2050-2059, régime réel normal) est plus rare et plus
     * ambigu avec un simple nombre à 4 chiffres (montant,
     * année...) : on exige alors un suffixe lettre ou "SD".
     */
    /\b(2032[\s\-]?(?:SD)?)\b/i,
    /\b(2033[\s\-]?[A-Z]{0,2}(?:[\s\-]?SD)?)\b/i,
    /\b(203[4-9][\s\-]?(?:[A-Z]{1,2}|SD))\b/i,
    /\b(20[45]\d[\s\-]?(?:[A-Z]{1,2}|SD))\b/i,
...
function buildFiscalDocumentType({
  ...
  if (
    ...
    /*
     * Seuls 2031, 2035, 2065 et 2042 sont des déclarations à
     * part entière ; le reste de la plage (2032-2039, 2050-2059)
     * désigne une ANNEXE de la liasse (détermination du résultat,
     * bilan, compte de résultat...) — les présenter comme "la"
     * déclaration serait trompeur.
     */
    const isMainDeclaration =
      /^(2031|2035|2065|2042)/i.test(
        formRef.value
      );

    return isMainDeclaration
      ? `Déclaration de résultats — formulaire ${formRef.value}`
      : `Liasse fiscale — annexe ${formRef.value}`;
  }
