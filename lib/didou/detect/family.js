...
export function detectDocumentFamily(input) {
   ...
   * télécom, salle de sport... N'importe quel type de
   * contrat peut être résilié.
   *
   * Sans ce candidat, une lettre de confirmation de
   * résiliation qui mentionne aussi un dernier prélèvement
   * (très fréquent : dernier mois facturé au prorata) se
   * fait classer comme simple "avis de prélèvement" — le
   * signal de paiement, secondaire, l'emporte alors que la
   * résiliation est le vrai sujet du courrier.
   */

  {
    ...
    if (
      /confirmation de resiliation|confirmation d.annulation|demande de resiliation|votre resiliation|votre contrat (?:est|sera) resilie|votre abonnement (?:est|sera) resilie|votre ligne sera desactivee|fin de votre abonnement|mettre fin a votre (?:contrat|abonnement)/.test(
        lower
      )
    ) {
      score += 45;
      ...
      signals.push(
        "structure:cancellation-confirmed"
      );
    }

    if (
      /resiliation|annulation|desactivee|desactive/.test(
        title
      )
    ) {
      score += 35;
    ...
    /*
     * Un simple rappel des conditions de résiliation dans
     * un document par ailleurs très clairement une facture
     * ne doit pas suffire à basculer la classification.
     */

    if (
      score > 0 &&
      invoiceSignals.score >= 70 &&
      !signals.includes(
        "structure:cancellation-confirmed"
      )
    ) {
      ...
      signals.push(
        "negative:strong-invoice-context"
      );
    }
...
function detectInsuranceSignals({
   ...
   * chômage" sont les noms légaux des branches de la Sécurité
   * sociale française — pas de l'assurance privée (contrats,
   * primes, sinistres) que ce détecteur cherche à repérer. Un
   * relevé de carrière CNAV ou un décompte CPAM ne doivent pas
   * gagner des points "assurance" simplement parce que ce mot
   * apparaît dans leur en-tête légal.
   */

  const lowerWithoutSocialInsurance =
    lower.replace(
      /assurance\s+(?:maladie|vieillesse|chomage|invalidite|deces)/g,
      ""
    );

  if (
    /\bassurance\b|\bassureur\b/.test(
      lowerWithoutSocialInsurance
    )
  ) {
    score += 25;
  ...
  const titleWithoutSocialInsurance =
    String(title || "").replace(
      /assurance\s+(?:maladie|vieillesse|chomage|invalidite|deces)/gi,
      ""
    );

  const headWithoutSocialInsurance =
    String(head || "").replace(
      /assurance\s+(?:maladie|vieillesse|chomage|invalidite|deces)/gi,
      ""
    );
  ...
  if (
    /assurance|assureur/.test(
      titleWithoutSocialInsurance
    ) ||
    /assurance|assureur/.test(
      headWithoutSocialInsurance
    )
  ) {
    score += 15;
