...
export function buildUserFacingExplanation(
  ...
  if (
    ...
    /*
     * Le type/la famille du document n'a pas pu être établi
     * avec confiance, mais si un fait concret et vérifié
     * existe malgré tout (une échéance, un montant, une
     * action...), Didou en sait déjà assez pour être utile —
     * "je n'ai rien compris" serait alors un mensonge par
     * excès de prudence.
     */
...
/*
 * Quand une fiche précise du catalogue de connaissances a été
 * reconnue avec confiance (consensus.summary n'est renseigné que
 * dans ce cas — voir brain/consensusEngine.js), sa description
 * est toujours plus utile que les phrases génériques par intention
 * ("Ce document définit ou confirme une relation contractuelle.",
 * "Ce document sert à certifier ou justifier une situation."...).
 * On la préfère donc, mais seulement après les phrases construites
 * à partir de faits réellement extraits (réunion, décision,
 * situation financière), qui restent plus précises encore.
 */

function catalogSummarySentence(
  consensus
) {
  const summary =
    cleanSentence(
...
function buildMainSentence({
  ...
  if (
    ...
    return phraseAsOneSentenceAction(
      action
    );
  }
...
function buildMeetingSentence({
  ...
  /*
   * Un rendez-vous médical, une audience ou un entretien ne
   * sont pas des "réunions" — appeler chaque événement par son
   * vrai nom évite une phrase correcte mais imprécise (et un
   * peu déplacée pour une convocation médicale ou judiciaire).
   */
  ...
  if (
    ...
    normalizedFamily ===
      "juridique" ||
    /judiciaire|audience|tribunal/.test(
      normalizedType
    )
  ) {
    eventName =
    ...
    normalizedFamily ===
      "social" ||
    /france travail|pole emploi/.test(
      normalizedType
    )
  ) {
    eventName =
  ...
  /*
   * Accord du participe "prévu(e)" avec le genre de eventName
   * ("Un rendez-vous" est masculin, "Une réunion" est féminin) —
   * jamais deviné autrement qu'à partir de l'article qu'on vient
   * nous-mêmes de choisir ci-dessus.
   */
  ...
  if (
    ...
    return ensureSentence(
      `${eventName} est ${prevu} le ${dateTime.date} à ${dateTime.time}, à ${place}`
    );
  }
  ...
  if (
    ...
    return ensureSentence(
      `${eventName} est ${prevu} le ${dateTime.date} à ${dateTime.time}`
    );
  }
  ...
  if (
    ...
    return ensureSentence(
      `${eventName} est ${prevu} le ${dateTime.date}, à ${place}`
    );
  }
  ...
  if (
    ...
    return ensureSentence(
      `${eventName} est ${prevu} le ${dateTime.date}`
    );
  }
...
function cleanMeetingPlace(
  ...
  /*
   * `\b` en JavaScript ne reconnaît pas les lettres accentuées
   * comme des caractères de mot : juste après "exprimé" (qui se
   * termine par un "é"), `\b` ne trouve jamais de frontière valide
   * et cette alternative ne matchait donc jamais — un `\b` placé
   * après une lettre normale (accentuée ou non) via une négation
   * anticipée fonctionne dans tous les cas.
   */

  text =
    text
      .replace(
        /\b(?:ordre du jour|résolution|resolution|vote exprimé|vote exprime|pouvoir|mandat|formulaire)(?![a-zà-ÿ]).*$/i,
        ""
      )
      .replace(
        /[,;:\-–—]+$/g,
...
function buildDecisionSentence({
  ...
  if (
    ...
    /*
     * Une fiche catalogue peut porter une nuance essentielle qu'un
     * simple montant ne peut pas transmettre — ex. un avis à tiers
     * détenteur adressé à un TIERS qui détient des fonds pour le
     * compte du débiteur, pas au débiteur lui-même : "Ce document
     * indique un montant de 3 240,00 € à régler" laisserait croire
     * au destinataire qu'IL doit cette somme personnellement.
     */
    const knownPaymentSentence =
      catalogSummarySentence(
        consensus
      );

    if (
      knownPaymentSentence
    ) {
      return knownPaymentSentence;
    }
...
function buildProofSentence({
  ...
  if (
    ...
    return ensureSentence(
      /^[aeiouyhéèêàâîïôùû]/i.test(
        lowered
      )
        ? `Ce document sert d’${lowered}`
        : `Ce document sert de ${lowered}`
    );
  }
...
function isGenericExplanation(
  ...
  /*
   * "^ce document concerne" a longtemps été traité comme un début
   * de phrase forcément générique — mais plusieurs fiches du
   * catalogue (RSA, prime d'activité, procédure judiciaire, rupture
   * conventionnelle...) écrivent des résumés spécifiques et utiles
   * qui commencent légitimement par ces mots ("Ce document concerne
   * vos droits au RSA et peut indiquer un montant..."). Cette règle
   * les rejetait TOUJOURS au profit d'un residu bien plus vague
   * ("Informer le destinataire d'une décision."), sans qu'aucun
   * texte réellement générique dans le catalogue ne corresponde
   * encore à ce début de phrase.
   */
...
/**
 * =====================================================
 * ACTION → PHRASE COMPLÈTE
 * =====================================================
 *
 * Les phrases d'action viennent de sources hétérogènes :
 * certaines sont déjà des phrases complètes avec leur propre
 * sujet ("Vous devez...", "Nous vous mettons en demeure...",
 * "Merci de...", "Régler la somme de..."), d'autres sont des
 * fragments d'obligation passive sans sujet ("doit être
 * transmis avant le..."). Les préfixer TOUTES par "Ce document
 * vous demande de [action en minuscule]" — comme le faisait ce
 * code auparavant — produit une phrase grammaticalement cassée
 * dès que l'action est déjà une phrase complète (ex. "Ce
 * document vous demande de nous vous mettons en demeure de
 * régler..."). On ne préfixe donc que les vrais fragments sans
 * sujet, et on garde le reste tel quel.
 */

/*
 * "Ce document"/"Ces documents" sont masculins : le participe qui
 * suit "doit être"/"doivent être" dans le texte source accorde
 * parfois au féminin ou au pluriel selon le VRAI sujet ("cette
 * attestation doit être transmise") — sujet qu'on remplace ici par
 * un sujet générique masculin, d'où le réaccord. Même logique que
 * masculinizeParticiple() dans brain/fusion.js (dupliquée ici pour
 * rester cohérente avec le style local du fichier).
 */
...
function isAlreadyCompleteActionSentence(
  action
) {
  return (
    /^(vous|nous|merci|veuillez|je|il\s|elle\s|ce\s|cette\s|ces\s|le\s|la\s|les\s|l['’])/i.test(
      action
    )
  );
}

function phraseAsOneSentenceAction(
  action
) {
  const text =
    cleanText(
  ...
  if (
    isAlreadyCompleteActionSentence(
      text
    )
  ) {
    return ensureSentence(
  ...
  return ensureSentence(
    `Ce document vous demande de ${lowerFirst(text)}`
  );
}
