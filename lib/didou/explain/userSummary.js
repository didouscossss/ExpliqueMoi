 ...
 * IMPORTANT V4.1 :
 *
 * L'ancien userSummary ne récupérait essentiellement
 * que :
 *
 * decision.primaryDate.value
 * partial.mainDate.date
 *
 * L'heure pouvait donc exister dans Fusion sans jamais
 * être affichée.
 *
 * On inspecte maintenant plusieurs emplacements
 * possibles sans modifier les données originales.
 */
...
const amount =
  ...
  if (
    ...
    if (
      ...
      addImportantPoint({
        ...
        seen,
        value:
          "Une action de votre part peut être nécessaire.",
        mainSentence
      });
    }
  }
   ...
   * ATTENTION :
   *
   * on ne reprend plus automatiquement l'ancien
   * decision.actionRequired = false lorsqu'un consensus
   * a corrigé le sens du document.
   */
  ...
  for (
    ...
    /*
     * On bloque un ancien fait "assurance"
     * lorsqu'un consensus fiable a corrigé le document
     * vers la copropriété / réunion.
     */

    if (
      shouldRejectLegacyFact({
        fact,
        value,
        consensus
      })
...
function getEffectiveActionRequired({
  ...
  /*
   * ===================================================
   * CONSENSUS A CORRIGE LE SENS
   * ===================================================
   *
   * Exemple :
   *
   * ancienne décision :
   * proof / actionRequired false
   *
   * consensus :
   * meeting / convocation
   *
   * Le false historique n'est plus fiable.
   */
