...
/**
 * Reconstruit le texte PDF en préservant les sauts de ligne via la coordonnée Y.
 * Évite d’aplatir toute la page en une seule ligne (ce qui faisait taguer
 * « capital social » sur tous les montants).
 */
function textContentToLines(items) {
  ...
  // PDF : Y décroît souvent vers le bas ; regroupe par proximité Y.
  rows.sort((a, b) => {
    const yDiff = b.y - a.y;
