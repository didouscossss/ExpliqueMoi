...
export function mapV3ResponseToUiAnalysis(v3Response) {
  ...
  if (ttc != null) {
    amounts.push({
      value: formatAmount(ttc),
      label: invoiceLike ? "TTC / à payer" : "TTC",
      kind: "ttc"
    });
  }
  ...
  // Facture / devis : montant principal = TTC / à payer en priorité (jamais un HT partiel).
  let principalValue = "Non trouvé";
  let principalMeaning = "Aucun montant identifié.";
  if (invoiceLike) {
    if (ttc != null) {
      principalValue = formatAmount(ttc);
      principalMeaning = "Montant TTC / à payer détecté localement.";
    } else if (ht != null) {
      principalValue = formatAmount(ht);
      principalMeaning = "Montant HT (TTC / à payer non détecté).";
    }
  } else if (ttc != null) {
    principalValue = formatAmount(ttc);
    principalMeaning = "Montant TTC détecté localement.";
  } else if (ht != null) {
    principalValue = formatAmount(ht);
    principalMeaning = "Montant HT détecté localement.";
  }
