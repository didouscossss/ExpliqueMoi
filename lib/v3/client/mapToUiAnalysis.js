...
export function mapV3ResponseToUiAnalysis(v3Response) {
  ...
  const vatRate = fields.vatRate ?? null;
  const invoiceDate =
    fields.invoiceDate || fields.issueDate || fields.date || null;
  ...
  // Dates sémantiques : facture d’abord (Date principale), prélèvement à part.
  const dates = [];
  if (invoiceDate) {
    dates.push({
      date: clean(invoiceDate),
      type: "invoice",
      label: "Date de facture",
      meaning: "Date d’émission / document"
    });
  }
  if (debitDate && debitDate !== invoiceDate) {
    dates.push({
      date: clean(debitDate),
      type: "debit",
      label: "Date de prélèvement",
      meaning: "Date de prélèvement / paiement"
    });
  }
  if (dueDate && dueDate !== debitDate && dueDate !== invoiceDate) {
    dates.push({
      date: clean(dueDate),
      type: "deadline",
      label: "Échéance",
      meaning: "Date d’échéance"
    });
  }
  for (const item of local.dates || []) {
    const iso = clean(item.iso || item.raw);
    if (!iso || dates.some((d) => d.date === iso)) continue;
    dates.push({
      date: iso,
  ...
  for (const deadline of local.deadlines || []) {
    const iso = clean(deadline.iso || deadline.raw);
    if (!iso || dates.some((d) => d.date === iso)) continue;
    dates.push({
      date: iso,
      type: "deadline",
  ...
  const mainDate = invoiceLike
    ? {
        date: clean(invoiceDate) || (dates[0] && dates[0].date) || "",
        meaning: "Date de facture",
        label: "Date de facture"
      }
    : {
        date: clean(fields.date) || (dates[0] && dates[0].date) || "",
        meaning: (dates[0] && dates[0].meaning) || "Date du document",
        label: (dates[0] && dates[0].label) || "Date"
      };

  // Passages importants = preuves LOCALES verbatim (jamais keyPoints IA).
  // Dédup normalisée (même texte, champs différents → une entrée).
  ...
  const evidence = localEvidence
    ...
    .filter((item) => {
      ...
      const norm = item.quote
        ...
        .replace(/\s+/g, " ")
        .trim();
      if (!norm || seenEvidence.has(norm)) return false;
      seenEvidence.add(norm);
      return true;
    });
  ...
  let principalSource = fields.principalSource || null;
  const principalReasons = Array.isArray(fields.principalReasons)
    ? fields.principalReasons
    : [];
  ...
  if (invoiceLike) {
    ...
    principalSource = picked.source || principalSource;
  ...
  // Warnings soft (parser / IA) exclus de la liste affichable (pas dans le résumé).
  const softWarning =
    /non détecté|clairement détecté|Aucun montant|indisponible|Analyse V3|faits locaux/i;
  const hardWarnings = warnings.filter((item) => !softWarning.test(item));
  ...
  return {
    ...
    urgency: {
      level:
        debitDate || (local.deadlines || []).length ? "soon" : "none",
      message:
        debitDate || (local.deadlines || []).length
          ? "Une échéance / date de prélèvement a été détectée."
